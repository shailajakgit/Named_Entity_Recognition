...
class Config:
    ...
    max_length = 32#120#32 # longest sequence to parse
    n_classes = 5
    dropout = 0.2
    embed_size = 50
    hidden_size = 900
    ...
    def __init__(self, args):
        ...
        if "model_path" in args:
            ...
            self.output_path = "F:/Jupyter/NLP/assignment3/results/{}/{:%Y%m%d_%H%M%S}/".format(self.cell, datetime.now())
...
def pad_sequences(data, max_length):
    ...
    #zero_vector = [0] * Config.n_word_features
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(dtype=tf.int32,shape=(None,self.max_length, self.config.n_features),name="input_placeholder")
        ...
        self.dropout_placeholder = tf.placeholder(dtype=tf.float32,shape=None, name="dropout_placeholder")
    ...
    def create_feed_dict(self, inputs_batch, mask_batch, labels_batch=None, dropout=1):
        ...
        feed_dict = dict()
        '''
        test_list = list(inputs_batch)
        
        print("##################Writing input batch#######################")
        with open("F:/Jupyter/NLP/assignment3/inputs_batch.txt","w") as f:
            f.write(str(test_list))
        '''
        input_array = np.array([np.array(lstBatch) for lstBatch in inputs_batch],dtype=int)
        feed_dict[self.input_placeholder] = input_array

        if labels_batch is not None:
            feed_dict[self.labels_placeholder] = labels_batch
            '''
            print("*********************Writing labels batch**********************")
            with open("F:/Jupyter/NLP/assignment3/labels_batch.txt","w") as f:
                f.write(str(test_list))
            '''
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings_full = tf.nn.embedding_lookup(params=self.pretrained_embeddings,ids=self.input_placeholder)
        embeddings = tf.reshape(embeddings_full,shape=(-1,self.max_length, self.config.n_features * self.config.embed_size),name="embeddings_reshape")
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(name="output_weight",shape=(self.config.hidden_size,Config.n_classes),dtype=tf.float32,initializer=tf.contrib.layers.xavier_initializer())
        b_2 = tf.get_variable(name="b_2",shape=(self.config.n_classes),dtype=tf.float32,initializer=tf.initializers.zeros())
        shape_x = tf.shape(x)
        h = tf.zeros((shape_x[0],self.config.hidden_size))
        ### END YOUR CODE

        with tf.variable_scope("RNN") as sc:
            rnn_cell = RNNCell(self.config.n_features * self.config.embed_size, self.config.hidden_size)
            for time_step in range(self.max_length):
                ### YOUR CODE HERE (~6-10 lines)
                if time_step > 0:
                    sc.reuse_variables()
                batch_of_words = tf.slice(x,begin=[0,time_step,0],size=[-1,1,-1])
                reshaped_batch_of_words = tf.reshape(batch_of_words,shape=(-1,self.config.n_features * self.config.embed_size))
                o_t, h = rnn_cell(reshaped_batch_of_words, h,sc)
                o_t_dropout = tf.nn.dropout(o_t,keep_prob=1-dropout_rate,name="h_dropout")
                o_t_logits = tf.matmul(o_t_dropout, U) + b_2 
        ...
        preds = tf.reshape(new_preds,shape=(-1,self.max_length,self.config.n_classes))
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        masked_preds = tf.boolean_mask(preds,self.mask_placeholder,name="masking_predictions")#,axis=1
        masked_labels = tf.boolean_mask(self.labels_placeholder,self.mask_placeholder,name="masking_labels")#,
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=masked_labels,logits=masked_preds,name="masked_loss")) 
    ...
    def preprocess_sequence_data(self, examples):
        def featurize_windows(data, start, end, window_size = 1):
            ...
            """
            #print("****call to preprocess_sequence_data->featurize_windows*********")
            ret = []
            for sentence, labels in data:
        ...
        print("******************Writing windowed data to a file***************")
        with open("F:/Jupyter/NLP/assignment3/examples.txt","w") as f:
...
if __name__ == "__main__":
    ...
    #parser.set_defaults(func=do_test1)
    
    # python q2_rnn.py -dt "F:/Jupyter/NLP/assignment3/data/train.conll" -dd "F:/Jupyter/NLP/assignment3/data/dev.conll" -v "F:/Jupyter/NLP/assignment3/data/vocab.txt" -vv "F:/Jupyter/NLP/assignment3/data/wordVectors.txt" -c "gru"
    parser.add_argument('-dt', '--data-train', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/train.conll", help="Training data")
    parser.add_argument('-dd', '--data-dev', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/dev.conll", help="Dev data")
    parser.add_argument('-v', '--vocab', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/vocab.txt", help="Path to vocabulary file")
    parser.add_argument('-vv', '--vectors', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/wordVectors.txt", help="Path to word vectors file")
    parser.add_argument('-c', '--cell', choices=["rnn", "gru"], default="rnn", help="Type of RNN cell to use.")
    parser.set_defaults(func=do_train)
    '''
    #parser = subparsers.add_parser('test2', help='')
    parser.add_argument('-dt', '--data-train', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/tiny.conll", help="Training data")
    parser.add_argument('-dd', '--data-dev', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/tiny.conll", help="Dev data")
    parser.add_argument('-v', '--vocab', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/vocab.txt", help="Path to vocabulary file")
    parser.add_argument('-vv', '--vectors', type=argparse.FileType('r'), default="F:/Jupyter/NLP/assignment3/data/wordVectors.txt", help="Path to word vectors file")
    parser.add_argument('-c', '--cell', choices=["rnn", "gru"], default="rnn", help="Type of RNN cell to use.")
