...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        # A list of tuples. Each tuple has two elements - gradient of loss w.r.t a variable and the variable, respectively.
        gradients = optimizer.compute_gradients(loss=loss,var_list = tf.trainable_variables(),aggregation_method = tf.AggregationMethod.ADD_N)
        # - Remember to clip gradients only if self.config.clip_gradients
        # is True.
        print("#############################################################")
        print("Just computed gradients")
        print(gradients)
        print("#############################################################")
        
        if self.config.clip_gradients:
            # Extract the gradients alone from [(grad1,var1),(grad2,var2),(grad3,var3),...]
            gradient_list = list()
            for g,v in gradients:
                gradient_list.append(g)

            # Clip the gradients
            list_clipped,global_norm = tf.clip_by_global_norm(
                            t_list=gradient_list,
                            clip_norm=self.config.max_grad_norm,
                            name="clipping_gradients")
            print("#############################################################")
            print("Clipped gradients")
            print(list_clipped)
            print("#############################################################")
            print("Recombining gradients gradients")

            # Get the var list from gradients to pass to apply_gradients
            var_list = list()
            for g,v in gradients:
                var_list.append(v)

            # Get the grads and var list to pass to apply_gradients
            clipped_gradients = zip(list_clipped,var_list)
            print(clipped_gradients)
            print("#############################################################")
                                                  
        if self.config.clip_gradients:
            train_op=optimizer.apply_gradients(clipped_gradients,name="apply_gradients")
        else:
            train_op=optimizer.apply_gradients(gradients,name="apply_gradients")

            # Compute gradients for global norm
            gradient_list = list()
            for g,v in gradients:
                gradient_list.append(g)
            global_norm=tf.linalg.global_norm(gradient_list,name="global_norm")
...
if __name__ == "__main__":
    ...
    #python q3_gru.py -c "rnn" -g
    #python q3_gru.py -c "gru" -g
